...
fn main() {
    ...
    fn open_serial(state: State<AppData>) -> bool {
        ...
        state_gaurd.port = serial_wraper::init_port(&state_gaurd.port_name, state_gaurd.baud_rate);
    ...
    fn get_ports() -> Vec<String> {
        return serial_wraper::list_ports();
    }
    ...
    fn send_serial(state: State<AppData>, input: String) -> bool {
        ...
        let write = serial_wraper::write_serial(&mut state_gaurd.port, input.as_str());
        match write {
            Ok(_) => {
    ...
    fn receive_serial(state: State<AppData>) -> String {
        ...
        match serial_wraper::wait_receive_serial(&mut state_gaurd.port) {
            Ok(s) => {
                println!("read found!");
                return s;
            }
